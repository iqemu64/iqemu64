...
{
    if(env->pc == (target_ulong)g_dyld_funcs.stub_binding_helper) {
        /* old fashion binder */
    ...
    } else if(env->pc == (target_ulong)g_dyld_funcs.dyld_stub_binder) {
        /* new fashion binder */
...
TranslationBlock *tb_gen_code(CPUState *cpu,
                              ...
                              uint32_t flags, int cflags, const char *types)
{
    CPUArchState *env = cpu->env_ptr;
    ...
    if(types && jit) {
        qemu_log("Should not have called here.\n");
        abort();
    }
 ...
 tb_overflow:
    ...
    if(jit) {
        ...
        } else if(pc == QTAILQ_FIRST(&env->CFIhead)->pc) {
            tb->tc.ptr = tcg_ctx->code_gen_xloop_ret_tb;
        } else if(pc == (target_ulong)g_dyld_funcs.stub_binding_helper ||
                  pc == (target_ulong)g_dyld_funcs.dyld_stub_binder) {
            tb->tc.ptr = tcg_ctx->code_gen_x64_lazy_binder_tb;
        /* TODO: Does it have to put in the interpose region?
         * Problem: These functions check the return address to do certain
         * things. If we put them in the interpose region, they behave incorrectly
         * because of the extra function layer if tail call optimization is not
         * engaged.
         * BUT, we can write our own asm to force tail call. Beats being here,
         * a minor hotspot.
        } else if(pc == (target_ulong)setjmp) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call_arm_setjmp);
        } else if(pc == (target_ulong)_setjmp) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call__arm_setjmp);
        } else if(pc == (target_ulong)sigsetjmp) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call_arm_sigsetjmp);
        } else if(pc == (target_ulong)longjmp) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call_arm_longjmp);
        } else if(pc == (target_ulong)_longjmp) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call__arm_longjmp);
        } else if(pc == (target_ulong)siglongjmp) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call_arm_siglongjmp);
        } else if(pc == (target_ulong)dlsym) {
            tb->tc.ptr = spec_x86func_code_gen((target_ulong)call__dlsym);
         */
        }
        /*
         * Here begins the main evils.
         * TODO: generate these tbs in the beginning of the process.
         */
        else if(pc == (target_ulong)g_objc_msg_funcs.objc_msgSend) {
            tb->tc.ptr = spec_x64func_code_gen((target_ulong)my_objc_msgSend);
        } else if(pc == (target_ulong)g_objc_msg_funcs.objc_msgSendSuper) {
            tb->tc.ptr = spec_x64func_code_gen((target_ulong)my_objc_msgSendSuper);
        } else if(pc == (target_ulong)g_objc_msg_funcs.objc_msgSendSuper2) {
            tb->tc.ptr = spec_x64func_code_gen((target_ulong)my_objc_msgSendSuper2);
        }
        /*
         * Misc functions.
         */
            ...
            /*
             * Since this is a minor hotspot, we may want to do the symbol name
             * search real fast. On the other hand, most of the symbol names have
             * already got in place in the lazy binder, I think this makes it 
             * hardly a hotspot now. Maybe we can do it throughly and slowly?
             * TODO: Get most of the symbol names in the lazy binder.
             */
            const char *symbol_name = solve_symbol_name(pc, NULL);
...
void
cpu_xloop_helper(fnEntryTranslation entry_translation, fnExitTranslation exit_translation, target_ulong *regs)
{
...
static void objc_add_trampoline(target_ulong m, void *trampoline)
{
    g_hash_table_insert(tcg_ctx->objc_trampolines, (gpointer *)m, trampoline);
}

static void *objc_query_trampoline(target_ulong m)
{
    return g_hash_table_lookup(tcg_ctx->objc_trampolines, (gpointer *)m);
}

uint8_t *objc_query_and_add_trampoline(CPUState *cpu, target_ulong m, uint32_t mark)
{
    mmap_lock();
    
    uint8_t *p = objc_query_trampoline(m);
    if(NULL == p) {
        p = tcg_gen_objc_trampoline(tcg_ctx, (Method)m, mark);
        objc_add_trampoline((target_ulong)m, p);
    }
