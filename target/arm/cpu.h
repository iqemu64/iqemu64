...
typedef struct CFIEntry {
    ...
    QTAILQ_ENTRY(CFIEntry) link;
} CFIEntry;

typedef enum CODE_GEN_HINT {
    CODE_GEN_HINT_NONE = 0,
    CODE_GEN_HINT_OBJC_CLASS,
} CODE_GEN_HINT;

typedef unsigned __int128 ARMVfpReg;

typedef struct CPUARMParameterRegs {
    uint64_t xregs[9];
    ARMVfpReg zregs[8]  QEMU_ALIGNED(16);
} CPUARMParameterRegs;
...
#define FFR_PRED_NUM 16
    ...
    target_ulong stack_bottom;
    
    QTAILQ_HEAD(, CFIEntry) CFIhead;
    
    struct {
        CODE_GEN_HINT code_gen_hint_type;
        uint64_t pc;
        union {
            void *cls;
                // Actually the type is id, let's not introduce troubles from ARC, just use primitive types.
        };
        void *handler;
        size_t gen_code_size;
    } code_gen_hints;
    bool arm_binding_lazy_symbol;
    const char *lazy_symbol_name;
} CPUARMState;

static inline
void save_arm_parameter_regs(const CPUARMState *env, CPUARMParameterRegs *saved)
{
    memcpy(saved->xregs, env->xregs, sizeof(saved->xregs));
    saved->zregs[0] = *(ARMVfpReg *)&env->vfp.zregs[0].d[0];
    saved->zregs[1] = *(ARMVfpReg *)&env->vfp.zregs[1].d[0];
    saved->zregs[2] = *(ARMVfpReg *)&env->vfp.zregs[2].d[0];
    saved->zregs[3] = *(ARMVfpReg *)&env->vfp.zregs[3].d[0];
    saved->zregs[4] = *(ARMVfpReg *)&env->vfp.zregs[4].d[0];
    saved->zregs[5] = *(ARMVfpReg *)&env->vfp.zregs[5].d[0];
    saved->zregs[6] = *(ARMVfpReg *)&env->vfp.zregs[6].d[0];
    saved->zregs[7] = *(ARMVfpReg *)&env->vfp.zregs[7].d[0];
}

static inline
void load_arm_parameter_regs(CPUARMState *env, const CPUARMParameterRegs *saved)
{
    memcpy(env->xregs, saved->xregs, sizeof(saved->xregs));
    *(ARMVfpReg *)&env->vfp.zregs[0].d[0] = saved->zregs[0];
    *(ARMVfpReg *)&env->vfp.zregs[1].d[0] = saved->zregs[1];
    *(ARMVfpReg *)&env->vfp.zregs[2].d[0] = saved->zregs[2];
    *(ARMVfpReg *)&env->vfp.zregs[3].d[0] = saved->zregs[3];
    *(ARMVfpReg *)&env->vfp.zregs[4].d[0] = saved->zregs[4];
    *(ARMVfpReg *)&env->vfp.zregs[5].d[0] = saved->zregs[5];
    *(ARMVfpReg *)&env->vfp.zregs[6].d[0] = saved->zregs[6];
    *(ARMVfpReg *)&env->vfp.zregs[7].d[0] = saved->zregs[7];
}
...
int cpu_arm_signal_handler(CPUState *cpu, int host_signum, void *pinfo,
                           void *puc);
