...
#define sigorset(set, left, right)     *(set) = *(left) | *(right)
                                                 ...
                                                 const siginfo_t *info)
{
    ...
    default:
        ...
        case TARGET_SIGIO:
            ...
            break;
        default:
            si_type = QEMU_SI_KILL;     // XXX: Fix me.
            break;
        }
        break;
    }
                                ...
                                void *puc)
{
    ...
    if ((host_signum == SIGSEGV || host_signum == SIGBUS)
        && info->si_code > 0) {
        if (cpu_signal_handler(cpu, host_signum, info, puc))
            return;
    }
...
void host_to_target_sigaction(struct target_sigaction *d, const struct sigaction *s) {
    if (!s || !d)
        return;
    d->_sa_handler = (abi_ulong)s->sa_handler;
    d->sa_flags = (abi_ulong)s->sa_flags;
    host_to_target_sigset(&d->sa_mask, &s->sa_mask);
}

void target_to_host_sigaction(struct sigaction *d, const struct target_sigaction *s) {
    if (!s || !d)
        return;
    d->sa_handler = (void (*)(int))s->_sa_handler;
    d->sa_flags = (int)s->sa_flags;
    target_to_host_sigset(&d->sa_mask, &s->sa_mask);
}
